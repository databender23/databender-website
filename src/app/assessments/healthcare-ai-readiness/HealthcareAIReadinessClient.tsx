...
export default function HealthcareAIReadinessClient() {
  ...
  const totalSteps = healthcareQuestions.length + 1; // +1 for contact form
  const progress = ((currentStep + 1) / totalSteps) * 100;

  const currentQuestion = healthcareQuestions[currentStep];
  const isLastQuestion = currentStep === healthcareQuestions.length - 1;
  const isContactStep = currentStep === healthcareQuestions.length;
  ...
  const handleAnswer = (value: number) => {
    ...
    setTimeout(() => {
      if (isLastQuestion) {
        setCurrentStep(healthcareQuestions.length);
      } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const scores = calculateHealthcareScores(answers);

    try {
      // Store results in sessionStorage for results page
      sessionStorage.setItem(
        "healthcareAIReadinessResults",
      ...
      router.push("/assessments/healthcare-ai-readiness/results");
    } catch (error) {
                ...
                <div className="mb-8">
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-teal-500 text-sm font-medium">
                      Question {currentStep + 1} of {healthcareQuestions.length}
                    </span>
              ...
              >
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  <Input
                    ...
                    onChange={handleContactChange}
                    placeholder="e.g., CEO, CIO, Director of Operations"
                  />
                  <div>
                    ...
                    <select
                      ...
                      value={contactInfo.organizationType}
                      onChange={handleContactChange}
                    ...
                    >
                      ...
                      <option value="clinic">Clinic / Ambulatory Care</option>
                      <option value="specialty">Specialty Practice</option>
                      <option value="long-term-care">Long-term Care / Senior Living</option>
                      <option value="home-health">Home Health</option>
                      <option value="health-plan">Health Plan / Payer</option>
