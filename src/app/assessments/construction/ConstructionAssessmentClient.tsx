...
import {
  ...
  calculateConstructionScores,
} from "@/lib/construction-assessment";

export default function ConstructionAssessmentClient() {
  ...
  const totalSteps = constructionQuestions.length + 1; // +1 for contact form
  const progress = ((currentStep + 1) / totalSteps) * 100;

  const currentQuestion = constructionQuestions[currentStep];
  const isLastQuestion = currentStep === constructionQuestions.length - 1;
  const isContactStep = currentStep === constructionQuestions.length;
  ...
  const handleAnswer = (value: number) => {
    ...
    setTimeout(() => {
      if (isLastQuestion) {
        setCurrentStep(constructionQuestions.length);
      } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const scores = calculateConstructionScores(answers);

    try {
      // Store results in sessionStorage for results page
      sessionStorage.setItem(
        "constructionAssessmentResults",
      ...
      router.push("/assessments/construction/results");
    } catch (error) {
        ...
        <div className="max-w-2xl mx-auto">
          ...
          >
            ...
            <h1 className="text-3xl md:text-4xl font-bold text-text-primary mb-4">
              Construction Data Readiness
            </h1>
            <p className="text-text-secondary">
              Find out where you stand and where better visibility would help most
            </p>
          </motion.div>
                ...
                <div className="mb-8">
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-teal-500 text-sm font-medium">
                      Question {currentStep + 1} of {constructionQuestions.length}
                    </span>
              ...
              >
                <div className="mb-8">
                  ...
                  <p className="text-text-secondary mt-2">
                    We&apos;ll email you a detailed report with recommendations
                    based on where you scored lowest.
                  </p>
                </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  <Input
                    ...
                    onChange={handleContactChange}
                    placeholder="e.g., CEO, CFO, Controller, VP Operations"
                  />
                  <div>
