...
export default function CREClient() {
  const router = useRouter();
  const [currentStep, setCurrentStep] = useState(0);
  const [profile, setProfile] = useState<CREProfile>({
    companyName: "",
    propertyCount: "",
    propertyTypes: [],
    externalInvestors: "",
  ...
  const assessmentSteps = creQuestions.length;
  ...
  const currentAssessmentQuestion = isAssessmentStep
    ? creQuestions[currentStep - profileSteps]
    : null;
  ...
  const canProceed = () => {
    if (isProfileStep && currentProfileQuestion) {
      ...
      if (q.type === "text") return (profile[q.id as keyof CREProfile] as string)?.length > 0;
      if (q.type === "select") return (profile[q.id as keyof CREProfile] as string)?.length > 0;
      if (q.type === "multiselect") return (profile[q.id as keyof CREProfile] as string[])?.length > 0;
    }
    return true;
  };
  ...
  const handleProfileMultiSelect = (questionId: string, value: string) => {
    setProfile((prev) => {
      const current = prev[questionId as keyof CREProfile] as string[];
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const scores = calculateCREScores(answers, profile);

    try {
      // Store results in sessionStorage for results page
      sessionStorage.setItem(
        "creAssessmentResults",
      ...
      router.push("/assessments/commercial-real-estate/results");
    } catch (error) {
  ...
  const roleOptions = [
    ...
    { value: "ceo-owner", label: "CEO / Owner" },
    { value: "coo-vp-ops", label: "COO / VP Operations" },
        ...
        <div className="max-w-2xl mx-auto">
          ...
          >
            ...
            <h1 className="text-3xl md:text-4xl font-bold text-text-primary mb-4">
              Portfolio Analytics Assessment
            </h1>
            <p className="text-text-secondary">
              See where your portfolio analytics stand and what centralized
              visibility could unlock
            </p>
          </motion.div>

          {/* Profile/Assessment/Contact Steps */}
            ...
            {isProfileStep && currentProfileQuestion ? (
              ...
              >
                    ...
                    {currentProfileQuestion.options?.map((option) => (
                      <button
                        ...
                        className={`w-full text-left p-4 rounded-xl border transition-all duration-200 ${
                          profile[currentProfileQuestion.id as keyof CREProfile] === option.value
                    ...
                    <div className="space-y-3 mb-6">
                      {currentProfileQuestion.options?.map((option) => (
                        <button
                          ...
                          className={`w-full text-left p-4 rounded-xl border transition-all duration-200 ${
                            (profile[currentProfileQuestion.id as keyof CREProfile] as string[])?.includes(
                              option.value
                            )
                              ? "bg-teal-500/10 border-teal-500"
                              : "bg-[#F8F9FA] border-black/10 hover:border-teal-500/50"
                ...
                {currentProfileQuestion.type === "text" && (
                  <>
                    <Input
                      value={(profile[currentProfileQuestion.id as keyof CREProfile] as string) || ""}
                      onChange={(e) =>
              ...
              >
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  <div>
                    <label className="block text-sm font-medium text-text-primary mb-2">
                      What&apos;s your biggest portfolio data challenge? (optional)
                    </label>
                    <textarea
