...
interface AgentNodeProps {
  ...
  onClick?: () => void
  showTooltip?: boolean
  usePlainEnglish?: boolean // V2: Added plain English toggle
}
...
export default function AgentNode({
  ...
  // V2: Choose label based on mode
  const displayLabel = usePlainEnglish ? config.shortLabel : config.shortLabel
  const tooltipLabel = usePlainEnglish ? config.plainEnglishLabel : config.label
  const tooltipDescription = usePlainEnglish ? config.plainEnglishDescription : config.description
        ...
        >
          <div className="flex items-center gap-2 mb-2">
            <div
              ...
              style={{ backgroundColor: config.color }}
            />
            <h4 className="font-semibold text-gray-900 text-sm">{tooltipLabel}</h4>
          </div>
          <p className="text-xs text-gray-600 leading-relaxed">{tooltipDescription}</p>
        </motion.div>
      )}
    </div>
  )
