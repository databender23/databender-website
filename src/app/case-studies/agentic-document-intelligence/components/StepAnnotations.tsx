    ...
    >
      <div className="flex items-center justify-between gap-4">
        <div className="flex-1 text-center p-3 bg-red-50 rounded-lg border border-red-200">
          ...
          <div className="text-lg font-bold text-red-600 mt-1">Hours per doc</div>
          <div className="text-xs text-red-500">+ Manual fixes</div>
        </div>
        ...
        <div className="flex-1 text-center p-3 bg-green-50 rounded-lg border border-green-200">
          ...
          <div className="text-lg font-bold text-green-600 mt-1">Minutes per doc</div>
          <div className="text-xs text-green-500">Fully automatic</div>
        </div>
      </div>
    </motion.div>
  )
...
export default function StepAnnotations({
  ...
  // V2: Use plain English versions
  const displayTitle = usePlainEnglish ? activeStep.plainEnglishTitle : activeStep.title
  const displayDescription = usePlainEnglish ? activeStep.plainEnglishDescription : activeStep.description
  ...
  return (
    <div className={`${className}`}>
        ...
        <div className="px-6 py-5">
          <AnimatePresence mode="wait">
            <motion.div
              ...
              transition={{ duration: 0.3 }}
            >
              <p className="text-text-secondary leading-relaxed">{displayDescription}</p>
      ...
      <p className="text-center text-xs text-text-muted mt-4">
        Press <kbd className="px-1.5 py-0.5 bg-bg-secondary rounded text-text-muted font-mono border border-border">&#8592;</kbd> or{' '}
        <kbd className="px-1.5 py-0.5 bg-bg-secondary rounded text-text-muted font-mono border border-border">&#8594;</kbd> to navigate
      </p>
    </div>
  )
}
