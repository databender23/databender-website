...
export default function RevealNumbers({ inView, className = '' }: RevealNumbersProps) {
  ...
  return (
    <div className={`text-center space-y-6 md:space-y-8 ${className}`}>
      ...
      >
        <span className="text-orange-500 font-semibold">1.5M records</span>
        <span className="mx-3 text-text-muted">{'\u2192'}</span>
        <span className="text-teal-500 font-semibold">100K unique customers</span>
      </motion.div>
      ...
      >
        Same people, different names - all resolved automatically
      </motion.p>
      ...
      <motion.div
        ...
        className="pt-6 grid grid-cols-3 gap-4 max-w-2xl mx-auto"
      >
        ...
        <div className="p-4 rounded-xl bg-bg-secondary border border-border">
          <p className="text-2xl font-bold text-text-primary">6 weeks</p>
          <p className="text-xs text-text-muted">Time to Complete</p>
        </div>
      </motion.div>
    </div>
  )
