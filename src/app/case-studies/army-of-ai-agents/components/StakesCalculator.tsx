...
export default function StakesCalculator({ inView, className = '' }: StakesCalculatorProps) {
  ...
  const recordsText = '1,689,749 records'
  ...
  return (
    <div className={`font-mono text-sm md:text-base ${className}`}>
      ...
      {/* Informative context - appears after animation */}
      <motion.div
        initial={{ opacity: 0, y: 20 }}
        animate={currentStep >= 6 ? { opacity: 1, y: 0 } : { opacity: 0, y: 20 }}
        transition={{ duration: 0.5 }}
        className="mt-6 p-4 rounded-xl bg-bg-secondary border border-border"
      >
        <p className="text-lg font-semibold text-text-primary text-center">
          This used to be a full-time job.
        </p>
        <p className="text-sm text-text-muted text-center mt-2">
          We&apos;ve seen analysts spend years on data like this. 44,421 collision groups. 99,581 unstable IDs. Every decision documented manually.
        </p>
      </motion.div>
    </div>
  )
