...
export default function ProcessingFlow({ isActive, className = '' }: ProcessingFlowProps) {
  ...
  // Progress documents through stages
  useEffect(() => {
    if (!isActive) return
    ...
    <div className={`${className}`}>
      <div className="bg-white rounded-2xl border border-slate-200 shadow-lg p-8">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            <h3 className="text-lg font-semibold text-slate-900">Document Processing Pipeline</h3>
            <p className="text-sm text-slate-500">Watch documents flow through the agent system</p>
          </div>
        ...
        <div className="relative py-8">
          {/* Connection line */}
          <div className="absolute top-1/2 left-0 right-0 h-1 bg-gradient-to-r from-slate-200 via-slate-300 to-slate-200 rounded-full -translate-y-1/2" />
          ...
          {stats.retries > 0 && (
            ...
            >
              ...
              <span className="text-sm text-amber-700">
                Self-healing: <strong>{stats.retries}</strong> errors auto-corrected
              </span>
            </motion.div>
          )}
        </AnimatePresence>
        ...
        <div className="mt-6 pt-6 border-t border-slate-100 grid grid-cols-4 gap-4">
          ...
          <div className="text-center">
            ...
            <div className="text-xs text-slate-500">Auto-Retried</div>
          </div>
          <div className="text-center">
            <div className="text-2xl font-bold text-green-500">0</div>
            <div className="text-xs text-slate-500">Manual Reviews</div>
          </div>
        </div>
      </div>
    </div>
